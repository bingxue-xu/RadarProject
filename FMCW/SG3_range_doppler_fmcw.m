%% Range-doppler Processing - FMCW Radar
% 
% Variation in results with no MS clutter rejection and no 2-pulse MTI.
% Variation in results with MS clutter rejection and no 2-pulse MTI.
% Variation in results by changing the length of the chunk of data being processed.
% Variation in results utilizing zero padding for both FFTs.
% 
% Group 7
% 10/09/2025
...
while Indx + chunk - 1 <= length(radar_backscatter)
    ...
    Y1 = ifft(data_2MTI, [], 2); % fast-time IFFT for range %
    ...
    %% No Zero Padding
    % Velocity axis
    d_f = linspace(0, 1/(2*Tp), size(Y2_lower,1));
    ...
    %% Plot frame
    imagesc(range, velocity, Y2_lower, [-50 0]); 
