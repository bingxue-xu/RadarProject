...
% For each rising index 'idx', take rng = idx : idx+N-1 (if within bounds).
valid_starts = rising(rising + N - 1 <= length(radar_data)); % discard starts that can't provide full chirp
M = length(valid_starts);  % number of full up-chirps

matrix_radar_up = zeros(M, N);
chirp_start_times = zeros(M,1); % optional: times of chirp starts (s)

for m = 1:M
    idx = valid_starts(m);
    matrix_radar_up(m, :) = radar_data(idx : idx + N - 1).'; % row = one chirp
    chirp_start_times(m) = (idx - 1) / Fs; % seconds since file start
end
