%% FMCW Radar
% 
% Simultaneous range and velocity estimation by only 
% processing the up and down chirp for a single target 
%
% Group 7
% Review Meeting 3
% 23/09/2025
...
xlabel('Range (m)');
ylabel('Time (s)');
title('Range[upc] with MS 3-pulse MTI, Tp=5ms, fstart=2.4GHz, fstop=2.5GHz');
...
xlabel('Range (m)');
ylabel('Time (s)');
title('Range[dwc] with MS 3-pulse MTI, Tp=5ms, fstart=2.4GHz, fstop=2.5GHz');
...
title('Beat frequency per up-chirp');
grid on;
...
title('Beat frequency per down-chirp');
grid on;
...
% Evaluating fifth order interpolation of datas
order = 5;
