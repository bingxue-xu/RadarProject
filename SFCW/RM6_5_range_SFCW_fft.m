...
L = floor(length(rx)/samples_per_cycle)*samples_per_cycle;
rx = rx(1:L);

rx3 = reshape(rx, samples_per_step, N, []);         % [samples, freq, cycles]
steady = round(0.5*samples_per_step):samples_per_step;
S_matrix = squeeze(mean(rx3(steady,:,:),1)).';      % M×N
M = size(S_matrix,1);

for m = 1:M
    base = (m-1)*samples_per_cycle;
    for n = 1: N
        seg = base + (n-1)*samples_per_step + steady;
        seg = seg + 1;
        seg(seg>length(rx)) = [];
...
% %% mean substraction
% S_matrix = S_matrix - mean(S_matrix, 2);
% 
% %% hann window
% w = hann(N).';
% S_matrix_hann = S_matrix .* w; 

%% ---- FFT ----
Nfft = 4*N;
R_spec = fft(S_matrix, Nfft, 2);
R_spec_dB = 20*log10(abs(R_spec)+eps);
R_spec_dB_half = R_spec_dB(:, 1:(Nfft/2));
%% normalization
max_R_spec = max(R_spec_dB_half(:));
R_spec_dB_half_norm = R_spec_dB_half - max_R_spec; 

%% range axis
R_axis = (0:Nfft/2-1) * (c/(2*Nfft*df));  % m
R_max = c/(2*df);
fprintf('Theoretical unambiguous range = %.2f m\n', R_max);

%% ---- TIME AXIS ----
% time_axis = (0:length(R)-1) * (N*Ts);  % one cycle = 2*Ts
time_axis = (0:M-1) * (N*Ts);

%% ---- PLOT RANGE OVER TIME ----
figure;
imagesc(R_axis, time_axis, R_spec_dB_half_norm);
axis xy; colorbar;
xlabel('Range (m)');
ylabel('Time (s)');
title('SFCW Range vs. Time (fft)');
grid on;

%% plot peak
[~, idx_pk] = max(R_spec_dB_half,[],2);
R_est = R_axis(idx_pk);

figure;
plot(time_axis, R_est, 'c.-', 'LineWidth', 1.2);
xlabel('Time(s)');
