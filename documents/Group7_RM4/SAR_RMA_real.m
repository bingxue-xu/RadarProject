...
[data,Fs] = audioread("finaltestSAR.wav");
radar_data = data(:,2);
sync_data = data(:,1);
...
N = floor(Tp * Fs);    % expected number of samples per upchirp
...
%% Robust grouping + protection against too-close starts
d = diff([0; sync(:); 0]);
starts_all = find(d == 1);
stops_all  = find(d == -1) - 1;
pulse_len_all = stops_all - starts_all + 1;
valid_mask = pulse_len_all >= (N - tol);

starts = starts_all(valid_mask);
stops  = stops_all(valid_mask);

% compute gaps between valid pulses (zeros between pulses)
...
if numel(gaps) >= 3
    ...
    try
        rng(0);
        idx = kmeans(X(:), 2, 'Replicates', 6, 'MaxIter', 1000);
        centers = [mean(X(idx==1)); mean(X(idx==2))];
        long_cluster = find(centers == max(centers), 1);
        gap_thresh = round(10^(centers(long_cluster)) - 1);
    catch
        % fallback
        gap_thresh = round(median(gaps) + 3*mad(gaps,1));
    end
else
    % too few gaps -> treat large gaps as the max gap found
    gap_thresh = round(max(gaps));
end
separators = find(gaps > gap_thresh);

%% GAP - method 1
% gap_thresh = 10 * Fs * Tp; % setting a threshold
% % separators = indices in 'gaps' where gap > threshold
% separators = find(gaps > gap_thresh);

%%

% split pulses into groups using separators
...
num_positions = numel(rail_positions);
disp(['Number of rail positions: ', num2str(num_positions)])
disp(['Start Indices of rail positions: ', num2str(rail_start_indices)])
...
for i = 1:length(rail_start_indices)
    mean_sync = mean(sync(rail_start_indices(i)+Nrp*3:rail_start_indices(i) + 4*Nrp - 1));
    if mean_sync > mean_thresh_min && mean_sync < mean_thresh_max
        radar_matrix(valid_rows,:) = radar_data(rail_start_indices(i)+Nrp*3:rail_start_indices(i) + 4*Nrp - 1)';
        sync_matrix(valid_rows,:) = sync(rail_start_indices(i)+Nrp*3:rail_start_indices(i) + 4*Nrp - 1)';
        valid_rows = valid_rows + 1;
    end
end

% CHANGED
radar_matrix = radar_matrix(1:valid_rows - 2, :);
sync_matrix = sync_matrix(1:valid_rows - 2, :);

% sync plot with final rail start indices
figure;
plot(sync);
hold on;
yl = ylim;
plot(starts, ones(size(starts))*yl(2)*0.98, 'g.', 'MarkerSize', 6); % pulse starts
for k = 1:length(rail_start_indices)
    xline(rail_start_indices(k), 'r--', 'LineWidth', 1.2);
end
title('Sync Signal with pulse starts (green) and final rail starts (red)');
xlabel('Sample Index');
ylabel('Sync Amplitude');
hold off;
...
%% k_x and 2-D matrix, k_t 2-D matrix, kz
...
%% Downrange and crossrange vector
